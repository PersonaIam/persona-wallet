...
function createWindow () {
  ...
  const iconpath = _path.resolve(__dirname, '/client/persona.png')
  ...
  ipcMain.on('ledger', (event, arg) => {
    if (arg.action === 'detect') {
      ...
      if (!ledgercomm) {
        ...
        try {
          let persona = new LedgerPersona(ledgercomm)
          if (arg.action === 'signMessage') {
            persona.signPersonalMessage_async(arg.path, Buffer.from(arg.data)
              .toString('hex'))
            ...
            persona.signTransaction_async(arg.path, arg.data)
          ...
          const result = {
            connected: false,
            message: 'Cannot connect to Persona application'
          }
          event.returnValue = result
        }
      }
  ...
  template = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: 'Learn More',
          click () { require('electron').shell.openExternal('https://persona.im') }
        },
