;(function () {
  ...
  function LedgerService ($q, $http, $timeout, storageService, networkService) {
    ...
    const personajs = require(_path.resolve(__dirname, '../node_modules/personajs'))
    ...
    function recoverBip44Accounts (backupLedgerPassphrase, slip44) {
      const hdnode = personajs.HDNode.fromSeedHex(bip39.mnemonicToSeedHex(backupLedgerPassphrase))
    ...
    function signTransaction (path, transaction) {
      return new Promise((resolve, reject) => {
        ...
        ipcRenderer.send('ledger', {
          ...
          data: personajs.crypto.getBytes(transaction, true, true).toString('hex'),
          path
        })
      })
    }
