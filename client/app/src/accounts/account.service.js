  ...
  function AccountService ($q, $http, networkService, storageService, ledgerService, gettextCatalog, gettext, utilityService, LAUNCH_DATE) {
    ...
    function createAccount (passphrase) {
      return new Promise((resolve, reject) => {
        const publicKey = persona.crypto.getKeys(passphrase).publicKey
        const address = persona.crypto.getAddress(publicKey, networkService.getNetwork().version)
    ...
    function isValidAddress (address) {
      return persona.crypto.validateAddress(address, networkService.getNetwork().version)
    }
    ...
    function verifyMessage (message, publicKey, signature) {
      ...
      const ecpair = persona.ECPair.fromPublicKeyBuffer(Buffer.from(publicKey, 'hex'))
      const ecsignature = persona.ECSignature.fromDER(Buffer.from(signature, 'hex'))
    ...
    function signMessage (message, passphrase) {
      ...
      let hash = crypto.createHash('sha256')
      hash = hash.update(Buffer.from(message, 'utf-8')).digest()
      const ecpair = persona.crypto.getKeys(passphrase)
    ...
    function createVirtual (passphrase) {
      const deferred = $q.defer()
      const address = persona.crypto.getAddress(persona.crypto.getKeys(passphrase).publicKey, networkService.getNetwork().version)
