...
const LedgerPersona = function (comm) {
  this.comm = comm
  this.comm.setScrambleKey('w0w')
}

/* eslint-disable node/no-deprecated-api */
LedgerPersona.prototype.getAddress_async = function (path) {
...
LedgerPersona.prototype.signTransaction_async = function (path, rawTxHex) {
  const splitPath = utils.splitPath(path)
...
LedgerPersona.prototype.getAppConfiguration_async = function () {
...
LedgerPersona.prototype.signPersonalMessage_async = function (path, messageHex) {
  const splitPath = utils.splitPath(path)
  let offset = 0
